...
class TelegramApprovals:
    def __init__(self, native, adapter, store, execute, prepare, review_context=None):
        ...
        native.add_handler(CallbackQueryHandler(self.scope_callback, pattern=r'^alfie_scope:[ar]:[0-9a-f]{32}$'), group=-101)
        native.add_handler(CommandHandler('alfie_approval_test', self.self_test), group=-100)
        import alfie_permissions
        alfie_permissions.SCOPE_CONFIRM = self.confirm_scope

    async def confirm_scope(self, mode, brief, *, read=False):
        from telegram import InlineKeyboardButton, InlineKeyboardMarkup
        from alfie_permissions import WRITES, READS
        if mode not in (READS if read else WRITES) or len(self.scope_pending) >= 4:
            return False
        review = json.dumps({'proposed_mode': mode, 'original_request': brief}, ensure_ascii=True, sort_keys=True)
        if len(review) > MAX_REVIEW:
            return False
        nonce = uuid.uuid4().hex
        future = asyncio.get_running_loop().create_future()
        record = {'future': future, 'message': None, 'read': read,
                  'expires': asyncio.get_running_loop().time() + 180}
        self.scope_pending[nonce] = record
        try:
            message = await self.native.bot.send_message(
                chat_id=self.policy['chat'], message_thread_id=self.policy['thread'] or None,
                text=(('Approve private read scope. No account changes.\n'
                      'Only this selector and IDs returned by this search may be read.\n'
                      'Check the exact query/resource against your original request.\n'
                      if read else 'Confirm task scope (not an account action).\n'
                      'This permits preparing only the proposed operation for review, with no other tools.\n'
                      'No Google change occurs without a SECOND, exact-action approval.\n') +
                      'Expires in 3 minutes or on restart. Original owner request below:\n\n' + review),
                parse_mode=None, disable_web_page_preview=True)
            record['message'] = str(message.message_id)
            await self.native.bot.edit_message_reply_markup(
                chat_id=self.policy['chat'], message_id=message.message_id,
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton('Approve read scope' if read else 'Confirm task scope', callback_data='alfie_scope:a:' + nonce),
                    InlineKeyboardButton('Cancel task', callback_data='alfie_scope:r:' + nonce)]]))
            return await asyncio.wait_for(future, timeout=max(0, record['expires'] - asyncio.get_running_loop().time()))
        except Exception:
            return False
        finally:
            self.scope_pending.pop(nonce, None)

    def confirm_read(self, operation, arguments, grant):
        binding = trusted_binding(self.policy)
        if binding['user'] != grant.owner or binding['chat'] != grant.chat \
                or binding['source_message'] != grant.message:
            raise ValueError('Read scope source mismatch')
        try:
            if asyncio.get_running_loop() is self.loop:
                raise ValueError('Read proposal must run in a tool worker')
        except RuntimeError:
            pass
        brief = {'owner_request': grant.brief, 'operation': operation, 'arguments': arguments}
        future = asyncio.run_coroutine_threadsafe(self.confirm_scope(grant.mode, brief, read=True), self.loop)
        try:
    ...
    async def decide_scope(self, query):
        ...
        if origin != self.policy or not self.adapter._is_callback_user_authorized(
            ...
            raise ValueError('Unauthorized scope callback')
        prefix, choice, nonce = query.data.split(':')
        record = self.scope_pending.get(nonce)
        if prefix != 'alfie_scope' or choice not in ('a', 'r') or not record \
                or record['message'] != str(message.message_id) or record['future'].done() \
                or record['expires'] <= asyncio.get_running_loop().time():
            raise ValueError('Scope review expired or mismatched')
        # Consume without yielding: duplicate concurrent callbacks cannot both confirm.
        self.scope_pending.pop(nonce)
        record['future'].set_result(choice == 'a')
        await query.answer(('Read scope approved' if record.get('read') else
                            'Task scope confirmed; exact-action approval still required') if choice == 'a' else 'Task cancelled')
        await query.edit_message_reply_markup(reply_markup=None)

    async def scope_callback(self, update, context):
        from telegram.ext import ApplicationHandlerStop
        try:
            await self.decide_scope(update.callback_query)
        except Exception:
            try:
                await update.callback_query.answer('Scope unavailable or expired. Start a new request.', show_alert=True)
            except Exception:
                pass
        finally:
