...
class TelegramTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def scope_request(self):
        task = asyncio.create_task(self.bridge.confirm_scope('gmail.send', 'Send a synthetic email'))
        for _ in range(10):
            await asyncio.sleep(0)
            if self.bridge.scope_pending and next(iter(self.bridge.scope_pending.values()))['message']:
                break
        return task, next(iter(self.bridge.scope_pending))

    async def test_private_read_review_is_distinct_and_never_executes_a_write(self):
        task = asyncio.create_task(self.bridge.confirm_scope('email-read',
            {'owner_request': 'Find booking', 'operation': 'gmail.search',
             'arguments': {'query': 'booking', 'max': 20}}, read=True))
        for _ in range(10):
            await asyncio.sleep(0)
            if self.bridge.scope_pending and next(iter(self.bridge.scope_pending.values()))['message']:
                break
        nonce = next(iter(self.bridge.scope_pending))
        self.assertIn('Approve private read scope', self.native.bot.send_message.call_args.kwargs['text'])
        record = self.bridge.scope_pending[nonce]
        self.assertTrue(record['read'])
        record['future'].set_result(False)
        self.assertFalse(await task)
        self.execute.assert_not_called()

    async def test_scope_confirmation_is_single_use_and_does_not_execute(self):
        task, nonce = await self.scope_request()
        query = self.query(nonce)
        query.data = 'alfie_scope:a:' + nonce
        await self.bridge.decide_scope(query)
        self.assertTrue(await task)
        with self.assertRaises(ValueError):
            await self.bridge.decide_scope(query)
        self.execute.assert_not_called()
        self.assertIn('SECOND', self.native.bot.send_message.call_args.kwargs['text'])

    async def test_scope_wrong_identity_message_expiry_and_cancel(self):
        task, nonce = await self.scope_request()
        for change in ('user', 'chat', 'topic', 'message', 'bot', 'auth', 'expired'):
            query = self.query(nonce)
            query.data = 'alfie_scope:a:' + nonce
            if change == 'user': query.from_user.id = 999
            if change == 'chat': query.message.chat.id = 999
            if change == 'topic': query.message.message_thread_id = 99
            if change == 'message': query.message.message_id = 99
            if change == 'bot': query.from_user.is_bot = True
            self.adapter._is_callback_user_authorized.return_value = change != 'auth'
            record = self.bridge.scope_pending[nonce]
            original = record['expires']
            if change == 'expired': record['expires'] = 0
            with self.subTest(change=change), self.assertRaises(ValueError):
                await self.bridge.decide_scope(query)
            record['expires'] = original
            self.assertFalse(task.done())
        self.adapter._is_callback_user_authorized.return_value = True
        query = self.query(nonce)
        query.data = 'alfie_scope:r:' + nonce
        await self.bridge.decide_scope(query)
        self.assertFalse(await task)
        self.execute.assert_not_called()

    async def test_scope_delivery_failure_and_capacity_fail_closed(self):
        self.native.bot.send_message.side_effect = RuntimeError('synthetic failure')
        self.assertFalse(await self.bridge.confirm_scope('gmail.send', 'Synthetic'))
        self.assertFalse(self.bridge.scope_pending)
        self.bridge.scope_pending = {str(i): {} for i in range(4)}
        self.assertFalse(await self.bridge.confirm_scope('gmail.send', 'Synthetic'))
        self.execute.assert_not_called()
