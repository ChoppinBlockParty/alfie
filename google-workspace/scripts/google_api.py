...
def _walk_parts(part: dict):
    """Yield every part in the MIME tree. Real mail nests: multipart/mixed wraps a
    multipart/alternative that holds the text. A one-level scan returns nothing."""
    yield part
...
def _attachment_names(msg: dict) -> list:
    """Filenames of attachments. A PDF receipt often carries the amount the body omits."""
    return [p["filename"] for p in _walk_parts(msg.get("payload", {}))
            if p.get("filename")]
